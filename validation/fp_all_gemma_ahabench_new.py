...
os.makedirs('validate_gemma_ahabench_new', exist_ok=True)

exp_list = [0.8, 0.9, 0.95, 0.97, 1.0]
gemma_eval_fol = 'sparsified_aha_gemma_new'
prompt_file = 'promptsGemmaAha.txt'
NUM_LAYERS = 26
NUM_FEATURES = 16384

def load_prompts(path):
    pattern = r"---\s*Prompt #(\d+) \(Slug: prompt_\d+_slug\) ---\s*\n<bos>(.*?)(?=\n---\s*Prompt #\d+ \(Slug: prompt_\d+_slug\) ---|\Z)"
    with open(path, 'r', encoding='utf-8') as f:
        text = f.read()
    matches = re.findall(pattern, text, re.DOTALL)
    return {int(prompt_no): prompt_text.strip() for prompt_no, prompt_text in matches}

prompt_dictionary = load_prompts(prompt_file)
...
def generation(prompt, selected_nodes, exponent, prompt_no):
    ...
    save_file = 'generations_ahaobj_gemma_e' + str(exponent) + '_prompt_no_' + str(prompt_no) + '.pkl'
    with open(os.path.join('validate_gemma_ahabench_new', save_file), 'wb') as f:
        pickle.dump(obj, f)
...
for gemma_file in os.listdir(gemma_eval_fol):
    ...
    if value in exp_list:
        ...
        generation(prompt_dictionary[extract_prompt_no], get_neurons, value, extract_prompt_no)