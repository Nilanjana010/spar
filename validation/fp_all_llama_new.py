...
os.makedirs('validate_llama', exist_ok=True)

exp_list = [0.8, 0.9, 0.95, 0.97, 1.0]
llama_eval_fol = 'sparsified_eval_llama2_largeexp'
prompt_file = 'promptsLlama.txt'
NUM_LAYERS = 16
NUM_FEATURES = 32768
...
def generation(prompt, selected_nodes, exponent, prompt_no):
    ...
    save_file = 'generations_obj_llama_e' + str(exponent) + '_prompt_no_' + str(prompt_no) + '.pkl'
    with open(os.path.join('validate_llama', save_file), 'wb') as f:
        pickle.dump(obj, f)
