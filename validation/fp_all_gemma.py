...
os.makedirs('validate_gemma', exist_ok=True)

exp_list = [0.8, 0.9, 0.95, 0.97, 1.0]
gemma_eval_fol = 'sparsified_eval_gemma2_largeexp'
prompt_file = 'promptsGemma.txt'
NUM_LAYERS = 26
NUM_FEATURES = 16384
...
def generation(prompt, selected_nodes, exponent, prompt_no):
    ...
    save_file = 'generations_obj_gemma_e' + str(exponent) + '_prompt_no_' + str(prompt_no) + '.pkl'
    with open(os.path.join('validate_gemma', save_file), 'wb') as f:
        pickle.dump(obj, f)
