...
os.makedirs('validate_llama_ahabench', exist_ok=True)

exp_list = [0.8, 0.9, 0.95, 0.97, 1.0]
llama_eval_fol = 'sparsified_aha_llama_largeexp'
prompt_file = 'promptsLlamaAha.txt'
NUM_LAYERS = 16
NUM_FEATURES = 32768

def load_prompts(path):
    pattern = r"--- Prompt #(\d+) \(Slug: prompt_\d+_slug\) ---\n<\|begin_of_text\|>(.*?)(?=\n--- Prompt #\d+|\Z)"
...
def generation(prompt, selected_nodes, exponent, prompt_no):
    ...
    save_file = 'generations_ahabench_obj_llama_e' + str(exponent) + '_prompt_no_' + str(prompt_no) + '.pkl'
    with open(os.path.join('validate_llama_ahabench', save_file), 'wb') as f:
        pickle.dump(obj, f)
